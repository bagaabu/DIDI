...
X=importdata('../didi_train_data_w.csv');
Y=importdata('../didi_train_label.csv');
data=[X Y];
N_orig=size(data,1);
...
for j=1:66
    ...
    % train
    tr_X=X1(1:N_tr,1:211);
    tr_lb=Y1(1:N_tr);
    % test
    ts_X=X1(N_tr+1:end,1:211);
    ts_lb=Y1(N_tr+1:end);
    % save test
    ts_X_ext=[ts_X;zeros(592-size(ts_X,1),211)];
...
writetable(best_weights_table, 'best_weights2.csv');
